...
async function parseGenericSdmxRows(buffer, enrichRowFn, timeDimensionId, collectedOutput, id, config, funcParams) {
    ...
    try {
      ...
      for (let i = 0; i < buf.length; i += CHUNK_SIZE) {
        ...
        if (rows.length > config.batch) {
          await flushRows(rows, collectedOutput, id, part++, config, funcParams);
        }
      }
      saxParser.close();
...
async function parseStructureSpecificRows(buffer, enrichRowFn, timeDimensionId, collectedOutput, id, config, funcParams) {
  return new Promise(async (resolve, reject) => {
    const saxParser = new SaxesParser({ xmlns: false });
    let rows = [], part = 1;
...
async function fetchDatasetRows(dataset, filters = {}, enrichRow, timeDimensionId, collectedOutput, id, config, funcParams) {
  ...
  if (isStructureSpecific) {
    await parseStructureSpecificRows(buf, enrichRow, timeDimensionId, collectedOutput, id, config, funcParams);
  } else {
    await parseGenericSdmxRows(buf, enrichRow, timeDimensionId, collectedOutput, id, config, funcParams);
  }
}
